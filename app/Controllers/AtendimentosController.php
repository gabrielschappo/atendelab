...
class AtendimentosController
{
    ...
    public function criar(): void
    {
        ...
        $data = $_POST['data_atendimento'] ?? date('Y-m-d');
        $horario = $_POST['horario_atendimento'] ?? date('H:i');
        ...
        $sql = "INSERT INTO atendimentos (pessoa_id, tipo_atendimento_id, usuario_id, descricao, status, data_atendimento, horario_atendimento)
        ...
        $this->json(['mensagem' => 'Atendimento registado com sucesso.', 'id' => $this->pdo->lastInsertId()], 201);
    }
    ...
    public function alterarStatus(): void
    {
        ...
        $sql = "UPDATE atendimentos SET status = :status, observacao_final = :observacao WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
            'id' => $id,
            'status' => $status,
